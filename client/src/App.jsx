...
export default function App() {
  ...
  const handleRoleSwitch = (role) => {
    ...
    addLog(`Switched viewing role to ${role}`, role);
  };

  const addLog = (action, roleOverride = null) => {
    const newLog = {
      id: Date.now(),
      userId: currentUser.username.split(' ')[0],
  ...
  // Tiered transaction fee calculator as outlined in page 16 of report:
  // Below RM1,000 = 1% fee | RM1,000 - RM5,000 = 2% fee | Above RM5,000 = 3% fee
  ...
  const handlePlaceBid = (e) => {
    ...
    // Update application state to notify that offers are ready
    ...
    // Search the applicant target email
    ...
    sendEmailNotification(
      ...
      `Excellent news! ${currentUser.institution} has submitted a tailored DANA financing offer for your business for RM ${newBidForm.amount} @ ${newBidForm.interestRate}% interest rate.`
    );
  ...
  const handleSelectOffer = (offerId, appId) => {
    ...
    // Convert applicant status to Approved & Repaying phase
    setApplications(prev => prev.map(app => {
      if (app.id === appId) {
        return {
          ...app,
    ...
    sendEmailNotification(
      ...
      `Dear team, MSME Danish Hakim has accepted your offer bid of RM ${selectedOffer.amount}. Disbursement is scheduled.`
    );
  };
  ...
  const handleProcessPayment = () => {
    ...
    setTimeout(() => {
      ...
      sendEmailNotification(
        ...
        `Dear Danish, your monthly installment of RM ${paymentData.loan.monthlyRepayment} has been cleared successfully. DANA Platform transaction fee: RM ${paymentData.feeAmount.toFixed(2)}.`
      );
    }, 2000);
  };
    ...
    <div className="min-h-screen bg-slate-900 text-slate-100 flex flex-col font-sans selection:bg-amber-500 selection:text-slate-900">
      ...
      <div className="bg-gradient-to-r from-amber-600 via-amber-500 to-amber-600 text-slate-950 px-4 py-2 flex flex-wrap gap-3 justify-between items-center text-sm font-semibold shadow-md z-50">
        <div className="flex items-center gap-2">
          <span className="bg-slate-900 text-amber-400 text-xs px-2 py-0.5 rounded-full font-mono uppercase tracking-wider font-extrabold">DANA Sandbox Mode</span>
          <span><strong>Project Proposal Presentation Sandbox:</strong> Switch roles to test dynamic workflows!</span>
        </div>
        <div className="flex items-center gap-1.5 bg-slate-950/20 p-0.5 rounded-lg border border-slate-950/10">
          <button 
            onClick={() => handleRoleSwitch('Applicant')}
            className={`px-3 py-1 rounded-md transition-all text-xs font-bold ${currentRole === 'Applicant' ? 'bg-slate-900 text-amber-400 shadow-sm' : 'text-slate-900 hover:bg-slate-950/10'}`}
          >
            MSME Applicant (Danish)
          </button>
          <button 
            onClick={() => handleRoleSwitch('Loan Officer')}
            className={`px-3 py-1 rounded-md transition-all text-xs font-bold ${currentRole === 'Loan Officer' ? 'bg-slate-900 text-amber-400 shadow-sm' : 'text-slate-900 hover:bg-slate-950/10'}`}
          >
            Loan Officer (Aniq - Maybank)
          </button>
          <button 
            onClick={() => handleRoleSwitch('Administrator')}
            className={`px-3 py-1 rounded-md transition-all text-xs font-bold ${currentRole === 'Administrator' ? 'bg-slate-900 text-amber-400 shadow-sm' : 'text-slate-900 hover:bg-slate-950/10'}`}
          >
            Platform Admin
          </button>
        </div>
              ...
              <div className="space-y-8">
                  ...
                  <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    <div>
                      ...
                      <p className="text-slate-400 mt-2 max-w-xl text-sm leading-relaxed">
                        Welcome, <strong>{currentUser.username}</strong>. Explore matching schemes, request digital loans, clear installment payments, and view dynamic banking bids on your company.
                      </p>
                    </div>
                    <button 
                      onClick={() => setActiveTab('apply')}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  <div className="bg-slate-950 p-6 rounded-2xl border border-slate-850 shadow-md">
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <p className="text-sm font-bold text-slate-200 mt-0.5">{currentUser.company}</p>
                      </div>
                      ...
                      <div>
                        ...
                        <p className="text-sm font-extrabold text-amber-400 mt-0.5">RM {currentUser.revenue.toLocaleString()}</p>
                      </div>
                    </div>

                    <div className="mt-6 pt-6 border-t border-slate-850">
                      <p className="text-xs text-slate-400 leading-relaxed mb-4">
                        *Need to update details? Adjust parameters in the upper development sandbox bar to test automatic qualification updates immediately.
                      </p>
                      <button 
                        onClick={() => {
                          setCurrentUser(prev => ({ ...prev, revenue: prev.revenue + 15000 }));
                  ...
                  <div className="lg:col-span-2 bg-slate-950 p-6 rounded-2xl border border-slate-850 shadow-md">
                    ...
                    {applications.filter(a => a.status === 'Repaying').length === 0 ? (
                      ...
                      <div className="space-y-6">
                        {applications.map((app, index) => {
                          if (app.status !== 'Repaying' || !app.activeLoan) return null;
                ...
                <div className="bg-slate-950 p-6 rounded-2xl border border-slate-850 shadow-md">
                  ...
                  {applications.filter(a => a.status === 'Bidding Open' || a.status === 'Offers Received').length === 0 ? (
                    ...
                    <div className="space-y-6">
                      {applications.filter(a => a.status === 'Bidding Open' || a.status === 'Offers Received').map(app => {
                    ...
                    <div key={app.id} className="bg-slate-950 rounded-2xl border border-slate-850 p-6 flex flex-col lg:flex-row justify-between gap-6">
                      ...
                      <div className="flex-1 space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4 max-w-md bg-slate-900 p-3 rounded-xl border border-slate-800 text-xs">
                          <div>
                            ...
                            <span className="font-bold text-slate-300">RM {app.revenue.toLocaleString()}</span>
                          </div>
                          <div>
                      ...
                      <div className="w-full lg:w-80 flex flex-col justify-between border-t lg:border-t-0 lg:border-l border-slate-850 pt-6 lg:pt-0 lg:pl-6">
                        <div>
                          ...
                          {/* Fit rating display based on active bank parameters */}
            ...
            {!paymentSuccess ? (
              <div className="space-y-4">
                <div className="bg-slate-950 rounded-2xl p-4 border border-slate-850 space-y-3">
                  ...
                  {/* Tie-in showing Danish and Aniq's tiered transaction fee logic proposal */}
                  <div className="flex justify-between items-center text-xs">
